...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!isset($_POST['id']) || empty($_POST['id'])) {
            throw new Exception("Error: No se proporcionó el ID de la rifa a editar.");
        }
        ...
        if (!isset($_POST['precio_boleto']) || empty($_POST['precio_boleto']) || !is_numeric($_POST['precio_boleto'])) {
            throw new Exception("Error: El precio del boleto es requerido y debe ser un valor numérico.");
        }
        ...
        if (isset($_FILES['imagen_rifa']) && $_FILES['imagen_rifa']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($fotoType, $validMimeTypes)) {
                throw new Exception("Error: La foto debe ser una imagen JPG, PNG o GIF.");
            }

            // Validar el tamaño de la imagen (máximo 5MB)
            if ($fotoSize > 5 * 1024 * 1024) {
                throw new Exception("Error: La foto no debe superar los 5MB.");
            }
            ...
            if (!move_uploaded_file($fotoTmpName, $fotoPath)) {
                throw new Exception("Error: No se pudo guardar la foto.");
            }
        } else {
